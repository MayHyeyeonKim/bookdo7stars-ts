...
const BlogBest = () => {
  ...
  useEffect(() => {
    dispatch(resetBooks());
    dispatch(getBooksByGroupNameRequest('BlogBest', page, 20));
  }, []);
  ...
  const handleClickMore = () => {
    ...
    // page += 1;
    setPage(nextPage);
    dispatch(getBooksByGroupNameRequest('BlogBest', nextPage, 20));
  };

  return (
    <>
      <Container data-testid="books-container" sx={{ width: '100vw', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <BooksContainerForGroupName books={books} title={'BlogBest Books'} />
